...
6. Lower throttle fully to **coast**. The bottom 1% is a motor cutoff: all motor commands go to zero, stabilization stops, and stick movements cannot power a motor until you raise throttle again. The rotors spin down naturally; existing motion, rotation, drag, and gravity continue. The HUD shows **COAST — MOTORS OFF** while armed at cutoff.
...
The starter Betaflight tune remains gentle and provisional for the inherited 50 ms motor lag. The default now uses **Acro with motor cutoff**: AirMode is off, MOTOR_STOP is on, and PID stabilization at minimum throttle is off. Betaflight clears its integral correction while throttle is off. Anti-gravity and throttle boost are disabled, so changing throttle no longer activates those additional boosts. Above cutoff, Betaflight still provides rate control; this is not a direct stick-to-motor mixer or an auto-level mode.

The bridge applies the cutoff immediately, even if the newest motor packet was sent before the stick reached zero. It also sends a true zero-throttle channel value within the 1% cutoff band so Betaflight's own motor-stop and PID-reset behavior agree. Motor spin-down and rigid-body motion remain physical; there is no artificial freeze or attitude reset. Raising throttle resumes powered control without requiring another arm-switch toggle, with a 0.4-second ramp from cutoff to soften the initial kick. The ramp acts on the throttle sent into Betaflight; it does not clamp its motor corrections or angular velocity. Once power is restored, throttle follows the stick normally; another cutoff takes effect immediately.
