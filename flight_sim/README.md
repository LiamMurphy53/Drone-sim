...
The controller now includes derivative damping and reduced integral gain, with integral relief during rapid stick changes on all three axes. Damping stays constant across the throttle range; stock D-min and throttle PID attenuation are disabled. Gyro and derivative filters use fixed cutoffs. Stick commands have no added rate-request slew limiter, so rapid opposite commands remain available. Maximum commanded rates and the expo curve are unchanged; physical angular velocity is never clamped. These changes address large-command overshoot and residual correction after a reversal without changing the physical motor response or the low-throttle coast/steering behavior. All gains remain provisional for these estimated plants.
